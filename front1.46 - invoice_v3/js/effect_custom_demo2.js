    ...
    if (active) {
        ...
        $( "#border_canvas" ).mousemove(function(){
            if(bool_zoom){
                ...
                $("#border_canvas").css('cursor', 'grabbing');
                $( "#rectoutput" ).css({"top" : outputY + Y2 + "px", "left": outputX + X2 + "px"});
            }
        });
    }else{
        ...
        WidthRect = 0;
        HeightRect = 0;
        $( "#rectoutput" ).css({"top" : YRect + "px", "left": XRect + "px","width" : WidthRect + "px", "height": HeightRect + "px"});

        $("#border_canvas").css('cursor', 'crosshair');

        XRect = event.offsetX;
        YRect = event.offsetY;
        $( "#rect" ).css({"top" : YRect + "px", "left": XRect + "px"});
...
$("#output").mouseup(function(){
    if (active) {
        ...
        if (event.offsetX > XRect && event.offsetY > YRect) {
            $( "#rectoutput" ).css({"top" : YRect + "px", "left": XRect + "px",
                "width" : WidthRect + "px", "height": HeightRect + "px"});
            outputX = XRect;
            outputY = YRect;
        }
        if (event.offsetX < XRect && event.offsetY < YRect) {
            $( "#rectoutput" ).css({"top" : YRect - HeightRect + "px", "left": XRect - WidthRect + "px",
                "width" : WidthRect + "px", "height": HeightRect + "px"});
            outputX = XRect - WidthRect;
            outputY = YRect - HeightRect;
        }
        if (event.offsetX < XRect && event.offsetY > YRect) {
            $( "#rectoutput" ).css({"top" : YRect + "px", "left": XRect - WidthRect + "px",
                "width" : WidthRect + "px", "height": HeightRect + "px"});
            outputX = XRect - WidthRect;
            outputY = YRect;
        }
        if (event.offsetX > XRect && event.offsetY < YRect) {
            $( "#rectoutput" ).css({"top" : YRect - HeightRect + "px", "left": XRect + "px",
                "width" : WidthRect + "px", "height": HeightRect + "px"});
            outputX = XRect;
            outputY = YRect - HeightRect;
        }
