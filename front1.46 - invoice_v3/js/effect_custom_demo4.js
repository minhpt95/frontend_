...
var XR1 = 0;
var YR1 = 0;
// kích thước và tọa độ tạm thời của rect 1 khi zoom
var deltaXR = 0;
var deltaYR = 0;
var deltaWR = 0;
var deltaHR = 0;
...
function zoom(event) {
	var perW = $('#Main_canvas').attr('width') / 10;
	var perH = $('#Main_canvas').attr('height') / 10;
	var Main_canvas_W = $('#Main_canvas').attr('width');
	var Main_canvas_H = $('#Main_canvas').attr('height');
    // console.log(1);
    ...
    if (isFirefox) {
        scale += event.deltaY * -0.033;
    }if (isChromium) {
        scale += event.deltaY * -0.001;
    }else{
        scale += event.deltaY * -0.001;
    }
    Main_canvas_W = (Main_canvas_W*scale);
    // console.log(scale);
    $( "#Main_canvas" ).css({"width" : Main_canvas_W + "px"});
    deltaYR = Yrect1*scale;
    deltaXR = Xrect1*scale;
    deltaWR = Wrect1*scale;
    deltaHR = Hrect1*scale;
    $('#rect1').css({"top" : deltaYR + "px", "left": deltaXR + "px","width" : deltaWR + "px","height" : deltaHR + "px"});
    
    // $('#Main_canvas').attr('width', 640);
