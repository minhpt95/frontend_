(function (root, factory) {
    ...
    function preRender(toggled) {
        ...
        // var color = $('.bt_rotate').val() + "deg"
        var color = $('.wrapper_output').attr('rotate') + "deg"
        $('.wrapper_color-active').each(function(){
            color = color + "," + $(this).find('.color').val();
        })
        $('.bt_output').css({
            'background-image': 'linear-gradient('+color+')'
        })
        $('.css_output').text('background-image: linear-gradient('+color+')')
    }
    $('.bt_rotate').on('keyup', function(){
        try{ preRender(); }catch(err){ }
    })
